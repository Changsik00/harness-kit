...
2. **규약 4(사후 검증)의 load-bearing 가정 미보강.** Consequences 가 "안전이 정지규칙·결정 로그·**사후 테스트의 품질에 전적으로 의존**"이라 명시했으나, phase-24 는 그 *발판*(post-commit-verify·정지규칙 엔진)만 깔고 *테스트 자체의 신뢰도*(GitHub #212 검증 0단계 revert/over-mock·뮤테이션·적대적 2차 검토)는 손대지 않았다. unattended 는 가짜 green 의 폭발 반경을 키우므로, 도그푸딩 전에 최소 검증 0단계 1건이 선행돼야 한다.
...
**후속 (phase-25 — auto 신뢰성)**: ① `AskUserQuestion` 리다이렉트 hook(auto 한정, 본 Addendum 1), ② #212 검증 0단계 사후 검증(본 Addendum 2), ③ auto e2e `test-e2e-auto-mode.sh`(산문만으로 충분한지 *측정*), ④ check-irreversible 차단 승격 + settings deny 층위 정합. 이 넷 이전의 auto 도그푸딩은 *검증되지 않은 자율*이다.
...
- 후속 phase-25 (auto 신뢰성): 본 Addendum 의 ①~④. GitHub #181(행동 기반 평가·bounded 수정 루프·가정 레지스트리), #212(테스트 신뢰 — 위험 비례 검증 단계).
