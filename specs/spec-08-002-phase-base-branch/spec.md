...
| **Spec ID** | `spec-08-002` |
| **Phase** | `phase-08` |
| **Branch** | `spec-08-002-phase-base-branch` |
...
- spec 간 의존성 처리 불가 — spec-08-001 결과물 위에서 spec-08-002를 작업하려면 main merge를 기다려야 함
...
sdd phase new work-model --base
  → state.json: baseBranch = "phase-08-work-model"
  → phase-08.md: Base Branch = phase-08-work-model
  (브랜치 생성 안 함)

첫 spec hk-ship 시점:
  → origin/phase-08-work-model 존재 확인
  → 없으면: git checkout -b phase-08-work-model main
            git push -u origin phase-08-work-model
  → PR target = phase-08-work-model

이후 spec들:
  → phase-08-work-model에서 분기
  → PR target = phase-08-work-model

모든 spec merge 후:
  → phase-08-work-model → main PR
```
...
- hk-ship의 `sdd archive` 강제 및 phase done 유도 (→ spec-08-003)
- hk-align NOW/NEXT/Icebox 출력 개선 (→ spec-08-004)
- phase-08-work-model → main 최종 PR 절차 (수동)

## ✅ Definition of Done

- [ ] 모든 단위 테스트 PASS
- [ ] `walkthrough.md` 와 `pr_description.md` 작성 및 archive commit
- [ ] `spec-08-002-phase-base-branch` 브랜치 push 완료 (→ `phase-08-work-model`)
- [ ] 사용자 검토 요청 알림 완료
