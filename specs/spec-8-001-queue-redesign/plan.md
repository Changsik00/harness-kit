# Implementation Plan: spec-8-001

## 📋 Branch Strategy

- 신규 브랜치: `spec-8-001-queue-redesign`
- 시작 지점: `main` (phase-8-work-model 브랜치는 hk-ship 시점에 just-in-time 생성)
- PR 타깃: `phase-8-work-model` (첫 hk-ship 시 자동 생성 후 타깃 지정)
...
### 수동 검증 시나리오
1. `sdd status` 실행 → `NOW: spec-8-001-queue-redesign`, `NEXT: spec-8-002-phase-base-branch` 출력 확인
