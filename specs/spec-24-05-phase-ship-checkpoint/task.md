...
### 1-2. 테스트 작성 (Red)
- [ ] `tests/test-decision-phase.sh`: fixture 에서 2개 spec walkthrough 에 `decision add`, `sdd decision list --phase` 가 양쪽 집계 + spec 라벨, 결정 없는 spec 스킵, 0건 graceful, 기존 `list` 불변
- [ ] 실행 → Fail 확인
- [ ] Commit: `test(spec-24-05): add failing test for phase decision rollup`
...
- [ ] `sources/bin/sdd`: `cmd_decision` list `--phase` → `_decision_list_phase` (active phase 전 spec walkthrough 집계, spec 라벨, graceful)
- [ ] `sources/` → `.harness-kit/` 미러링
- [ ] 테스트 실행 → Pass + 전체 회귀
- [ ] Commit: `feat(spec-24-05): sdd decision list --phase (rollup)`
