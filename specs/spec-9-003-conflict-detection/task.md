...
### 1-1. 브랜치 생성
- [x] `git checkout -b spec-9-003-conflict-detection` (phase-9-install-conflict-defense에서 시작)
- [x] Commit: 없음 (브랜치 생성만)
...
- [x] 테스트 실행 → Fail 확인
- [x] Commit: `test(spec-9-003): add failing test for path config system`
...
- [x] Commit: `refactor(spec-9-003): fix common.sh agent paths and add config reading`
...
- [x] Commit: `feat(spec-9-003): add prefix UX and harness.config.json to install.sh`
...
- [x] Commit: `refactor(spec-9-003): doctor.sh reflects harness.config.json paths`
