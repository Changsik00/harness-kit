# PR: spec-9-003 — install prefix UX + path config system
...
- `test(spec-9-003): add failing test for path config system`
- `refactor(spec-9-003): fix common.sh agent paths and add config reading`
- `feat(spec-9-003): add prefix UX and harness.config.json to install.sh`
- `refactor(spec-9-003): doctor.sh reflects harness.config.json paths`
