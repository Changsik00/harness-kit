...
#### [NEW] `sources/root/.env.telegram.example`
```text
TELEGRAM_BOT_TOKEN=
TELEGRAM_CHAT_ID=
```

#### [NEW] `sources/root/.env.discord.example`
```text
DISCORD_BOT_TOKEN=
DISCORD_CHANNEL_ID=
```
...
#### [MODIFY] `install.sh` — 루트 파일 복사 스텝 추가
bin 복사 스텝(§12) 인근에 새 스텝 추가. `sources/root/` 존재 시:
- `sources/root/*.sh` → `$TARGET/` 복사 (`do_cp`) + `chmod +x` (`do_run "chmod +x ..."` 로 dry-run 존중 — Critique 누락)
- `sources/root/.env.*.example` → `$TARGET/` 복사 (`do_cp`)
- 실제 `.env.telegram` / `.env.discord` 는 **절대 생성·덮어쓰지 않음**
- DRY_RUN 존중 (do_cp 가 이미 처리)
