...
- **phase-review C1 (독립 감사가 적발)**: W2 의 "빈 mode → block" 정당화 전제("empty 는 rare")가 *거짓*이었음 — `install.sh` 가 초기 state 에 `mode` 를 시드하지 않아 **모든 fresh install 이 빈 mode**(=CLI 는 governed 로 해석)인데 hook 은 block 으로 분류 → attended 과차단 회귀. 메인 세션이 코드 검증 없이 단정한 부분. 해소: install.sh 에 `mode:governed` 시드(전제를 *참으로* 만듦) + T4(시드)·T5(round-trip) 테스트 + hook 주석에 safety/display 차등 명시. → self-review blind spot 을 독립 감사가 잡은 사례.
