...
### 1-1. 브랜치 생성
- [ ] `git checkout -b spec-9-005-update-rewrite` (phase-9-install-conflict-defense에서 시작)
- [ ] Commit: 없음 (브랜치 생성만)
...
- [ ] `tests/test-update.sh` 작성:
  ...
  - doctor 통과 확인
- [ ] 테스트 실행 → Fail 확인
- [ ] Commit: `test(spec-9-005): add failing test for update.sh rewrite`
...
- [ ] 기존 테스트 회귀 확인
- [ ] Commit: `refactor(spec-9-005): rewrite update.sh as uninstall+install+cleanup`
...
- [ ] **Push**: `git push -u origin spec-9-005-update-rewrite`
