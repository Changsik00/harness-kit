...
- [x] Spec ID 확정 및 디렉토리 생성 (`specs/spec-8-002-phase-base-branch/`)
- [x] spec.md 작성
- [x] plan.md 작성
- [x] task.md 작성 (이 파일)
...
### 1-1. 브랜치 생성
- [ ] `git checkout -b spec-8-002-phase-base-branch phase-8-work-model`
- [ ] Commit: 없음 (브랜치 생성만)
...
- [ ] 테스트 실행 → Fail 확인
- [ ] Commit: `test(spec-8-002): add failing tests for sdd phase base branch support`
...
- [ ] 테스트 실행 → 4/4 Pass 확인
- [ ] Commit: `feat(spec-8-002): add --base flag to sdd phase new and baseBranch to status json`

### 2-2. sources/ 동기화
- [ ] `sources/bin/sdd` — `scripts/harness/bin/sdd` 와 동일 변경 반영
- [ ] Commit: `chore(spec-8-002): sync sources/bin/sdd with harness implementation`
...
- [ ] Commit: `feat(spec-8-002): add phase base branch jit creation spec to hk-ship step 4`
...
- [ ] **Archive Commit**: `docs(spec-8-002): archive walkthrough and pr description`
- [ ] **Push**: `git push -u origin spec-8-002-phase-base-branch`
- [ ] **사용자 알림**: 푸시 완료 + PR 생성 요청 (PR 타깃: `phase-8-work-model`)
