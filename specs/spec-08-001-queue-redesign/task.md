...
- [x] Spec ID 확정 및 디렉토리 생성 (`specs/spec-08-001-queue-redesign/`)
- [x] spec.md 작성
- [x] plan.md 작성
- [x] task.md 작성 (이 파일)
...
### 1-1. 브랜치 생성
- [x] `git checkout -b spec-08-001-queue-redesign`
- [x] Commit: 없음 (브랜치 생성만)
...
- [x] Commit: `docs(spec-08-001): define work type model in constitution`
...
- [x] Commit: `refactor(spec-08-001): redesign queue template with now-next-icebox structure`

### 2-2. phase.md 템플릿 갱신
- [x] `sources/templates/phase.md` — spec 표 상태 컬럼에 `In Progress` 추가, 메타에 `Base Branch` 행 추가
- [x] Commit: `refactor(spec-08-001): add in-progress state and base-branch field to phase template`
...
- [x] 테스트 실행 → Fail 확인
- [x] Commit: `test(spec-08-001): add failing tests for sdd status next computation and sdd queue output`
...
- [x] 테스트 실행 → 5/5 Pass
- [x] Commit: `feat(spec-08-001): add next-spec computation to sdd status and structured sdd queue output`
...
### 5-1. 현재 프로젝트 반영
- [x] `backlog/queue.md` — 신규 마커 구조로 수동 재작성 (NOW=spec-08-001, NEXT=spec-08-002 반영)
- [x] `scripts/harness/bin/sdd` — sources/bin/sdd와 동일 파일 (이미 반영됨)
- [x] 테스트 실행 → 5/5 Pass
- [x] Commit: `chore(spec-08-001): apply queue redesign dogfooding to current project`
...
- [x] **Archive Commit**: `docs(spec-08-001): archive walkthrough and pr description`
- [x] **Push**: `git push -u origin spec-08-001-queue-redesign`
