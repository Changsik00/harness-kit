# refactor(spec-08-001): 작업 분류 모델 정의 & Queue NOW/NEXT/Icebox 재설계
...
### Phase 컨텍스트
- **Phase**: `phase-08` (base branch 모드, 도그푸딩 첫 사례)
- **본 SPEC 의 역할**: 작업 관리 모델의 기반 정의 — spec-08-002(phase base branch), spec-08-003(완료 흐름 강제), spec-08-004(hk-align 강화)가 이 모델 위에 구축됨
...
### 수동 검증 시나리오
1. `sdd status` → `NEXT: spec-08-002-phase-base-branch` 출력 확인
...
- Walkthrough: `specs/spec-08-001-queue-redesign/walkthrough.md`
