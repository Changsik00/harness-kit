...
- `sdd phase new` / `sdd spec new`가 기존 phase 번호를 인식하지 못하고 자동 번호를 매기는 문제 발견 (phase-4가 이미 있는데 phase-6으로 생성). state 수동 조정으로 해결했으나 향후 `sdd` CLI 개선 후보.
