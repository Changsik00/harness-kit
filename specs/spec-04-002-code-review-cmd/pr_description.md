# feat(spec-04-002): /code-review 슬래시 커맨드 추가

## 📋 Summary

### 배경 및 목적
에이전트가 자기 코드를 자기가 검증하는 확인 편향을 완화하기 위해, 독립 sub-agent가 코드 변경을 리뷰하는 옵셔널 슬래시 커맨드를 추가한다. spec-04-001(`/spec-review`)이 기획 리뷰라면, 이 커맨드는 **코드 수준의 실질적 리뷰**에 집중한다.
...
### Phase 컨텍스트
- **Phase**: `phase-04` (옵셔널 Sub-agent 리뷰 시스템)
- **본 SPEC 의 역할**: phase-04의 두 번째 spec으로, 코드 수준 리뷰 기능을 제공
...
- Walkthrough: `specs/spec-04-002-code-review-cmd/walkthrough.md`
