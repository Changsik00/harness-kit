...
Spec-level ship(`/hk-ship`)은 archive → push → PR 생성까지 잘 정의되어 있다. 그러나 Phase-level ship(phase base branch → main 최종 PR)은 절차가 없다. phase-8에서 실제로 에이전트가 spec PR 4개의 제목을 나열한 것만으로 최종 PR을 생성하려 했고, 사용자가 이를 차단했다.
...
- [ ] `spec-8-005-phase-ship` 브랜치 push 완료 (→ `phase-8-work-model`)
- [ ] 사용자 검토 요청 알림 완료
