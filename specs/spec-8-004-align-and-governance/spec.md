# spec-8-004: hk-align 강화 & 거버넌스 통합 & README 최신화
...
| **Spec ID** | `spec-8-004` |
| **Phase** | `phase-8` |
| **Branch** | `spec-8-004-align-and-governance` |
...
- README가 phase-8 변경사항(NOW/NEXT/Icebox queue, phase base branch, archive completion gate, specx done 등)을 반영하지 않는다.
...
hk-align 상태 보고에 NOW/NEXT 섹션을 추가한다. agent.md에 작업 유형별 행동 규칙 표와 완료 체크리스트를 명문화한다. README를 phase-8까지의 변경사항으로 최신화한다.
...
- sdd status 출력 포맷 변경 (이미 spec-8-001에서 완료)
...
- [ ] `spec-8-004-align-and-governance` 브랜치 push 완료 (→ `phase-8-work-model`)
- [ ] 사용자 검토 요청 알림 완료
