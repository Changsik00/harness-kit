# spec-9-001: 디렉토리 레이아웃 마이그레이션 — .harness-kit/ 은닉 구조 전환
...
| **Spec ID** | `spec-9-001` |
| **Phase** | `phase-9` |
| **Branch** | `spec-9-001-dir-layout` |
| **Base Branch PR Target** | `phase-9-install-conflict-defense` (hk-ship 시 자동 생성) |
| **상태** | Planning |
| **타입** | Refactor |
...
`backlog/`, `specs/`는 이 스펙에서 변경하지 않는다 (spec-9-003 scope).
...
- CLAUDE.md @import 방식 전환 (spec-9-002)
- `backlog/`, `specs/` 경로 충돌 감지 및 config 시스템 (spec-9-003)
- install/update.sh preflight 문의 UX (spec-9-004)
...
- [ ] `spec-9-001-dir-layout` 브랜치 push 완료
- [ ] 사용자 검토 요청 알림 완료
