# refactor(spec-9-001): move all harness-kit artifacts to .harness-kit/ hidden directory
...
### Phase 컨텍스트
- **Phase**: `phase-9` — install-conflict-defense
- **본 SPEC 의 역할**: Phase 9의 첫 스펙으로, 가장 근본적인 충돌 원인인 디렉토리 네임스페이스 오염을 해결. 이후 spec-9-002(CLAUDE.md @import), spec-9-003(config 시스템), spec-9-004(preflight UX)의 기반 레이아웃을 확정함.
...
### 수동 검증 시나리오
1. **sdd 정상 작동**: `bash .harness-kit/bin/sdd status` → `harness-kit 0.4.0`, phase-9 active
...
- Walkthrough: `specs/spec-9-001-dir-layout/walkthrough.md`
