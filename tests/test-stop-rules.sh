...
run_hook() {
  local mode="$1" cmd="$2"
  # 패턴 감지 테스트는 모드를 명시 고정(env override) — repo state.mode 의존 제거.
  if [ "$mode" = "block" ]; then
    _out="$(CLAUDE_TOOL_INPUT_command="$cmd" HARNESS_HOOK_MODE_STOP_RULES=block bash "$HOOK" 2>&1 1>/dev/null)"; _rc=$?
  else
    _out="$(CLAUDE_TOOL_INPUT_command="$cmd" HARNESS_HOOK_MODE_STOP_RULES=warn bash "$HOOK" 2>&1 1>/dev/null)"; _rc=$?
  fi
}

# run_hook_default <state.mode> <command> → env override 없이 *기본* 모드 동작 검증.
# 모드 차등: auto → block(fail-safe), 그 외 → warn. (spec-25-04 후속 mode-diff)
run_hook_default() {
  local sm="$1" cmd="$2" d
  d=$(mktemp -d); FIXTURES_TO_CLEAN+=("$d")
  mkdir -p "$d/.claude/state"; printf '{"mode":"%s"}\n' "$sm" > "$d/.claude/state/current.json"
...
# T10b: block 모드 reset --hard → exit 2
run_hook block "git reset --hard HEAD~1"
if [ "$_rc" -eq 2 ] && echo "$_out" | grep -q "hook:block"; then
  ok "T10b: block 모드 reset --hard → exit 2"
else
  fail "T10b: block reset --hard exit 2 기대 (rc=$_rc)"
fi

# ── 모드 차등 기본값 (env override 없이) ─ auto=block(fail-safe) / attended=warn ──
# T20: auto 기본 → block(exit 2). unattended 라 정지가 fail-safe.
run_hook_default auto "git push --force origin main"
if [ "$_rc" -eq 2 ] && echo "$_out" | grep -q "hook:block"; then
  ok "T20: auto 기본 → block(exit 2) — 실제 정지"
else
  fail "T20: auto 기본 block 기대 (rc=$_rc out=$_out)"
fi
# T21: governed 기본 → warn(exit 0). 사람 옆 — 의도적 명령 차단 안 함.
run_hook_default governed "git push --force origin main"
if [ "$_rc" -eq 0 ] && echo "$_out" | grep -q "hook:warn"; then
  ok "T21: governed 기본 → warn(exit 0)"
else
  fail "T21: governed 기본 warn 기대 (rc=$_rc out=$_out)"
fi
# T22: turbo 기본 → warn(exit 0). attended.
run_hook_default turbo "git reset --hard HEAD~1"
if [ "$_rc" -eq 0 ] && echo "$_out" | grep -q "hook:warn"; then
