...
echo ""
echo "▶ Check 3: sdd queue — Icebox 섹션 헤더 존재"
check
QUEUE_OUT=$(cd "$FIXTURE_DIR" && $SDD_CMD queue 2>&1 || true)
if echo "$QUEUE_OUT" | grep -q "Icebox"; then
  pass "queue 출력에 Icebox 섹션 존재"
else
  fail "queue 출력에 Icebox 섹션 없음"
fi
...
if echo "$RAW_OUT" | grep -q "sdd:active:start"; then
