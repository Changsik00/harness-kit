...
| Phase 2: 골격 구축 | ✅ | |
| Phase 3: 툴킷 빌드 (8 SPEC) | ✅ | governance, templates, install/uninstall/update/doctor, slash, hooks, bin/sdd, docs 모두 완료 |
| Phase 4: 도그푸딩 (`nextmarket-api` 에 install) | ⏸ | 사용자 결정 대기 |
| Phase 5: 개선 루프 | ⏸ | 도그푸딩 결과로 환류 |

**v0.1.0** (alpha) — 첫 사용자(`nextmarket-api`) 도그푸딩 대기 중.

도그푸딩에서 발견되는 모든 버그/개선점은 키트로 환류됩니다. v0.2 부터는 *자기 자신을 자기로 관리* 합니다.
...
*만든 사람: dennis · AI 협업: Claude Code (Opus 4.6)*
*첫 도그푸딩 대상: [`nextmarket-api`](../../NextPay/nextmarket-api) — NestJS + StepPay 결제 플랫폼*
