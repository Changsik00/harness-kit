...
| 유형 | 역할 | PR? | 언제 쓰나 |
|---|---|:---:|---|
| **Phase** | 연관 Spec 묶음 (Epic) | — | 3개 이상의 Spec이 묶이거나, 통합 테스트가 필요한 큰 작업 |
...
- **One Task = One Commit** — task.md의 각 항목은 하나의 커밋에 대응합니다.
...
harness-kit 소스 레포지토리의 구조입니다. 대상 프로젝트에 설치되는 파일들의 원본입니다.

```
harness-kit/
├── README.md, CLAUDE.md, VERSION
├── install.sh       # 대상 프로젝트에 키트 설치
├── update.sh        # 기설치 키트 갱신
├── uninstall.sh     # 키트 제거 (산출물 보존)
├── doctor.sh        # 설치 상태 점검
├── cleanup.sh       # deprecated 파일 정리
│
├── sources/                      # 대상 프로젝트로 복사될 파일들
│   ├── governance/               #   constitution.md, agent.md, align.md
│   ├── templates/                #   spec, plan, task, walkthrough, pr_description, phase, queue 양식
│   ├── commands/                 #   슬래시 커맨드 11종 (hk- prefix)
│   ├── hooks/                    #   PreToolUse 후크 3종
│   ├── bin/                      #   sdd + lib/common.sh + lib/state.sh
│   └── claude-fragments/         #   settings.json / CLAUDE.md fragment
│
├── tests/
│   └── fixtures/                 #   키트 자체 검증용 임시 디렉토리
│
└── docs/
    ├── design/                   #   설계 근거 (Harness Engineering Review)
