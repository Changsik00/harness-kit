...
| **Shell** | zsh / bash | 사용자 환경은 zsh 가정, 스크립트는 `#!/usr/bin/env bash` |
...
│   │   ├── hk-align.md, hk-spec-new.md, hk-plan-accept.md
│   │   ├── hk-spec-status.md, hk-handoff.md
│   │   ├── hk-gh-pr.md, hk-bb-pr.md, hk-code-review.md
...
│
└── CLAUDE.md                       # HARNESS-KIT 블록 추가 (기존 내용 보존)
```

### 사용자 보존 (멱등성)
- `.claude/settings.json` 기존 `permissions`, `env` 는 **합쳐짐**, hooks 만 키트가 권위
- `CLAUDE.md` 사용자 내용 보존, HARNESS-KIT 블록만 추가
- 두 번째 install: 중복 없음
...
| 명령 | 설명 |
|---|---|
| `sdd status [--json]` | 현재 상태 |
| `sdd phase new <slug>` | 새 phase 생성 |
| `sdd spec new <slug>` | 새 spec 생성 |
...
| 커맨드 | 설명 |
|---|---|
| `/hk-align` | 세션 부트스트랩 (규약 로드 + 상태 보고) |
| `/hk-spec-new` | 새 SPEC + 템플릿 생성 |
| `/hk-plan-accept` | plan.md 승인 → Strict Loop 시작 |
| `/hk-spec-status` | 현재 진행 상태 (읽기 전용) |
| `/hk-handoff` | 검증 + archive + push |
| `/hk-gh-pr` | GitHub PR 생성 (gh CLI) |
| `/hk-bb-pr` | Bitbucket PR 생성 |
...
`/hk-bb-pr` 사용 시 Bitbucket App Password 가 필요합니다:
...
> GitHub 사용자는 `gh auth login` 만 하면 `/hk-gh-pr` 이 바로 동작합니다.
...
└───────────────────────────────────────────────┘
    ↓
/hk-handoff (archive + push + PR)
    ↓
Phase 의 모든 SPEC merge → 통합 테스트 → Done
```
...
A. `check-plan-accept.sh` hook 이 `Edit/Write/MultiEdit` 을 검사합니다. 안전 경로만 통과. constitution §4.3 의 Premature Execution = Critical Violation 을 코드로 강제합니다.
