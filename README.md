...
> 자세한 정의: `agent/constitution.md` §3 Work Type Model
...
2. `bin/sdd status` 로 현재 상태 확인 (NOW / NEXT 포함)
...
/hk-ship (archive + push + PR 생성)
    ↓
sdd archive → phase.md 자동 Merged 갱신
    ↓
모든 Spec merge → sdd phase done → 통합 테스트 → Done
```
