...
### 1-1. 브랜치 생성
- [x] `git checkout -b spec-09-09-preflight-ux` (from `phase-09-install-conflict-defense`)

### 1-2. 테스트 작성 (TDD Red)
- [x] `tests/test-preflight.sh` 작성 (5개 체크)
- [x] Commit: `test(spec-09-09): add preflight scan test cases`

### 1-3. install.sh preflight 구현 (TDD Green)
- [x] install.sh에 preflight 블록 inline 추가
- [x] 테스트 실행 → Pass
- [x] Commit: `feat(spec-09-09): add preflight scan to install.sh`
...
- [x] Commit: `feat(spec-09-09): add preflight scan to update.sh with state fallback`
...
- [ ] **Archive Commit**: `docs(spec-09-09): archive walkthrough and pr description`
- [ ] **Push**: `git push -u origin spec-09-09-preflight-ux`
- [ ] **PR 생성**: 사용자 승인 후
