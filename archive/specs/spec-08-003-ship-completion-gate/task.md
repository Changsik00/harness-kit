...
- [x] Spec ID 확정 및 디렉토리 생성 (`specs/spec-08-003-ship-completion-gate/`)
- [x] spec.md 작성
- [x] plan.md 작성
- [x] task.md 작성 (이 파일)
...
### 1-1. 브랜치 생성
- [ ] `git checkout -b spec-08-003-ship-completion-gate phase-08-work-model`
- [ ] Commit: 없음 (브랜치 생성만)
...
- [ ] 테스트 실행 → Fail 확인
- [ ] Commit: `test(spec-08-003): add failing tests for archive completion gate`
...
- [ ] Commit: `feat(spec-08-003): auto-update phase.md to merged and prompt phase done after archive`

### 2-2. sdd specx done 명령 추가
- [ ] `scripts/harness/bin/sdd` — `cmd_specx()` + `specx_done()` 추가
- [ ] `sdd` 진입점에 `specx) cmd_specx "$@" ;;` 등록
- [ ] 테스트 Check 4 → Pass 확인
- [ ] Commit: `feat(spec-08-003): add sdd specx done command for queue completion`

### 2-3. sources/ 동기화
- [ ] `sources/bin/sdd` — 위와 동일 변경 반영
- [ ] Commit: `chore(spec-08-003): sync sources/bin/sdd with harness implementation`
...
- [ ] Commit: `feat(spec-08-003): add spec-x queue completion step to hk-ship step 6`
...
- [ ] **Archive Commit**: `docs(spec-08-003): archive walkthrough and pr description`
- [ ] **Push**: `git push -u origin spec-08-003-ship-completion-gate`
