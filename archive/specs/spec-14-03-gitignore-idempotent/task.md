...
### 1-1. 브랜치 생성
- [x] `git checkout -b spec-14-03-gitignore-idempotent`
- [x] Commit: 없음 (브랜치 생성만)

### 1-2. phase-14.md sdd:specs 마커 보정
- [x] 마커 안에 spec-14-03 행 추가:
  ```
...
- [x] `git add specs/spec-14-03-gitignore-idempotent/`
- [x] Commit: `chore(spec-14-03): start spec — planning + phase specs marker fixup`
...
### 2-1. 테스트 스크립트 추가
- [x] 파일 생성: `tests/test-gitignore-idempotent.sh`
...
- [x] 실행 권한 부여: `chmod +x tests/test-gitignore-idempotent.sh`

### 2-2. Fail 확인 (TDD Red)
- [x] 실행: `bash tests/test-gitignore-idempotent.sh`
- [x] 기대 결과: D-1~D-8 + H 는 PASS (현재 install.sh 도 `.harness-kit/` 라인은 라인별 처리), E + F + G 가 FAIL (헤더 누락 / 사전 라인 / 라인 일부 누락 케이스)
- [x] Commit: `test(spec-14-03): add idempotency regression test for .gitignore`
...
- [x] `_gi_ensure()` 헬퍼 함수 도입 (블록 스코프)
- [x] 헤더 단독 라인별 grep + ensure
- [x] 4 라인 각각 라인별 grep + ensure
- [x] 토글 (`.harness-kit/` ↔ `!.harness-kit/`) sed 변환 + ensure

### 3-2. Pass 확인 (TDD Green)
- [x] 실행: `bash tests/test-gitignore-idempotent.sh`
- [x] 기대 결과: 모든 시나리오 (D, E, F, G, H) PASS
- [x] 회귀: `bash tests/test-gitignore-config.sh` PASS (기존 A~G 시나리오 영향 없음)
- [x] Commit: `fix(spec-14-03): make .gitignore idempotent at line level`
...
- [x] **Ship Commit**: `docs(spec-14-03): ship walkthrough and pr description`
- [x] **Push**: `git push -u origin spec-14-03-gitignore-idempotent`
