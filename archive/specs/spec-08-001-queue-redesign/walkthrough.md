...
```text
═══════════════════════════════════════════
 Queue Redesign Verification (spec-08-001)
═══════════════════════════════════════════

▶ Check 1: sdd status — NEXT 행 존재 여부
  ✅ sdd status 출력에 'NEXT:' 행이 있음
▶ Check 2: sdd status — NEXT 값이 spec-08-002인지 확인
  ✅ NEXT = spec-08-002 (첫 번째 Backlog spec)
...
1. **Action**: `sdd status` 실행
   - **Result**: `NEXT: spec-08-002-phase-base-branch` 출력 확인
...
- `sdd:now`/`sdd:next` 마커가 queue.md에 추가됐으나 `sdd plan accept`, `sdd archive` 시 자동 갱신 로직은 spec-08-003에서 구현 예정

## 🚧 이월 항목

- `update_queue_now_next()` 자동 갱신 (plan accept / archive 연동) → spec-08-003에서 구현
