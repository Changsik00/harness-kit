...
> [!IMPORTANT]
> - [ ] `sdd archive` 이름 재사용: spec-11-01에서 deprecated 경로로 유지 중인 `sdd archive`를 완전히 교체. 기존 deprecated 동작(= `sdd ship`) 대신 새 디렉토리 아카이브 동작으로 변경
...
| 컴포넌트 | 전략 | 이유 |
|:---:|:---|:---|
| **명령 이름** | `sdd archive` (재사용) | spec-11-01에서 deprecated → ship으로 이관 완료. "archive"가 디렉토리 이동의 자연스러운 이름 |
