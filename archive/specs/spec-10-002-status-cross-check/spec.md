# spec-10-002: sdd status 자기 진단 엔진
...
| **Spec ID** | `spec-10-002` |
| **Phase** | `phase-10` |
| **Branch** | `spec-10-002-status-cross-check` |
| **상태** | Planning |
| **타입** | Feature |
...
1. **브랜치 패턴 미해석**: `spec-10-001-archive-status-fix` 브랜치에 있어도 "이건 phase-10의 이전 spec 작업 브랜치"라는 판단을 못함. 에이전트가 유저에게 "머지 됐나요?", "main으로 갈까요?" 같은 불필요한 질문을 반복하게 됨.
...
- spec 산출물 완성도 검증 (spec-10-003 범위)
- `_check_phase_all_merged`, `compute_next_spec` 수정 (spec-10-004 범위)
- `--quiet` 플래그
...
- [ ] `spec-10-002-status-cross-check` 브랜치 push 완료
- [ ] 사용자 검토 요청 알림 완료
