...
### spec-11-01 — sdd archive → sdd ship 리네이밍
...
- **요점**: phase/spec 번호를 2자리 제로패딩으로 통일 (`phase-01`, `spec-01-01`) — 파일 정렬 보장
...
- **Given**: 기존 `phase-01` ~ `phase-11`, `spec-01-01` ~ `spec-10-05` 디렉토리 존재
- **When**: 마이그레이션 스크립트 실행
- **Then**: `phase-01` ~ `phase-11`, `spec-01-01` ~ `spec-10-05`로 변환, `ls` 정렬이 숫자 순서와 일치
- **연관 SPEC**: spec-11-02
...
| `sdd archive` 이름 재사용으로 인한 혼동 | 기존 사용자가 구 동작 기대 | spec-11-01에서 deprecation 경고 + 전환기간 확보 (구 명령 호출 시 안내) |
