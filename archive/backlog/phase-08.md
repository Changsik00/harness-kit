# phase-08: 작업 관리 모델 재정립 — Queue·Phase base branch·완료 흐름 강제

> 본 phase 의 모든 SPEC 을 한 파일에 요점/방향성으로 나열합니다.
> *구체적* 작업 내용은 `specs/spec-08-{seq}-{slug}/spec.md` 에서 다룹니다.
...
| **목표 종료일** | — |
| **소유자** | Dennis |
| **Base Branch** | `phase-08-work-model` (opt-in, 도그푸딩 첫 사례) |
...
| ID | 슬러그 | 우선순위 | 상태 | 디렉토리 |
|---|---|:---:|---|---|
| spec-08-001 | queue-redesign | P1 | Merged | `specs/spec-08-001-queue-redesign/` |
| spec-08-002 | phase-base-branch | P1 | Merged | `specs/spec-08-002-phase-base-branch/` |
| spec-08-003 | ship-completion-gate | P1 | Merged | `specs/spec-08-003-ship-completion-gate/` |
| spec-08-004 | align-and-governance | P2 | Merged | `specs/spec-08-004-align-and-governance/` |
| spec-08-005 | phase-ship | P1 | Merged | `specs/spec-08-005-phase-ship/` |
<!-- sdd:specs:end -->

### spec-08-001 — 작업 분류 모델 정의 & Queue 재설계
...
### spec-08-002 — Phase base branch 지원 (opt-in)
...
### spec-08-003 — 완료 흐름 강제 (hk-ship 재설계)
...
### spec-08-004 — hk-align 강화 & 거버넌스 통합
...
- **방향성**:
  ...
  4. phase-08 자체가 base branch 모드이므로 도그푸딩 검증 포함
- **연관 모듈**: `sources/commands/hk-align.md`, `sources/governance/agent.md`, `agent/agent.md`

## 🧪 통합 테스트 시나리오

> phase-08는 base branch 모드로 진행 — 통합 테스트는 phase-08 브랜치에서 실행 후 main merge.

### 시나리오 1: NOW/NEXT 출력 정확성
- **Given**: phase-08 진행 중, spec-08-001 완료(Merged), spec-08-002 In Progress
- **When**: `sdd status` 실행
- **Then**: `NOW: spec-08-002`, `NEXT: spec-08-003` 출력
- **연관 SPEC**: spec-08-001

### 시나리오 2: Phase base branch just-in-time 생성
- **Given**: phase-08가 base branch 모드, spec-08-002 작업 완료, phase-08 브랜치 미존재
- **When**: hk-ship 실행
- **Then**: phase-08 브랜치 자동 생성 → spec-08-002 PR이 phase-08 타깃으로 생성됨
- **연관 SPEC**: spec-08-002
...
- **Given**: phase-08 진행 중 + spec-x 하나 Icebox에 있음
- **When**: hk-align 실행
...
- **선행 phase**: phase-07 (완료)
- **외부 시스템**: 없음
- **spec 내부 의존성**: spec-08-001 → spec-08-002, 003 / spec-08-001, 002, 003 → spec-08-004
