...
1. `sdd archive` 실행 시 `| Done |` 상태의 spec도 `| Merged |`로 정상 전환된다. (spec-10-01 완료)
...
| ID | 슬러그 | 우선순위 | 상태 | 디렉토리 |
|---|---|:---:|---|---|
| `spec-10-01` | archive-status-fix | P1 | Merged | `specs/spec-10-01-archive-status-fix/` |
| `spec-10-02` | status-cross-check | P1 | Merged | `specs/spec-10-02-status-cross-check/` |
| `spec-10-03` | spec-completeness | P2 | Merged | `specs/spec-10-03-spec-completeness/` |
| `spec-10-04` | phase-done-accuracy | P2 | Merged | `specs/spec-10-04-phase-done-accuracy/` |
| `spec-10-05` | artifact-timing-fix | P? | Merged | `specs/spec-10-05-artifact-timing-fix/` |
<!-- sdd:specs:end -->
...
- **방향성**:
  ...
  - **잔재 감지 + 행동 제안**: 이전 spec의 미정리 항목(archive 누락, task.md 미체크) 감지 시 구체적 정리 명령 안내. 예: `⚠ spec-10-01: phase.md(Done) ↔ git(머지됨) → sdd archive 실행 권장`
  - 기본 출력에 진단 섹션 포함. `--brief`/`--json` 에서는 생략.
- **연관 모듈**: `sources/bin/sdd` (`cmd_status`), `sources/bin/lib/state.sh`

### spec-10-03 — spec 완성도 검증
...
- **Given**: 현재 브랜치가 `spec-10-01-archive-status-fix`
- **When**: `sdd status` 실행
