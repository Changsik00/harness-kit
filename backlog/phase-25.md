...
4. settings `deny`(never-justify, 완전 차단) ↔ `check-irreversible` ②(context-dependent, 멈추고 대기) **2층 모델 명문화** + 데드락(W3) 해소. **모드 차등 적용**(spec-25-04 후속, 2026-06-22): auto → block(fail-safe, 실제 정지), attended → warn. e2e 가 auto block 정지를 증명. (1주 단계론 정제 — CLAUDE.md #5)
