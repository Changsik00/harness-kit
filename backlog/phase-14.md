...
| `spec-14-05` | phase-review-followup | P? | Merged | `specs/spec-14-05-phase-review-followup/` |
<!-- sdd:specs:end -->
...
- [x] 모든 SPEC (spec-14-01 ~ 05) 이 main 으로 merge — PR #76, #77, #78, #79, #80 모두 머지
- [x] 통합 테스트 시나리오 4 건 모두 PASS (아래 "검증 결과" 참조)
- [x] 성공 기준 1~5 정량 측정 결과 기록
- [ ] 사용자 최종 승인 (go/no-go)

> 참고: 본 phase 는 Base Branch 미사용 — 5 spec 이 main 으로 직접 PR 머지됨. Phase PR 별도 생성 불필요.

## 📊 검증 결과 (2026-04-25 ~ 2026-04-26)

### 통합 시나리오 4건 — 모두 PASS

#### 시나리오 1: queue.md queued 섹션 정합성 (spec-14-01)
- **검증**: `grep -rn "sdd:queued" sources/templates/ .harness-kit/agent/templates/` → **0 매치**
- **결과**: ✅ Option B 채택. dead marker 제거 + 안내문 일관 (Icebox 와 동일 정책: 사람 편집).
...
- **결과**: ✅ **22/22 PASS** (D-1~8 재install / E 헤더 누락 / F 사전 라인 / G 라인 일부 누락 / H 토글)

#### 시나리오 4: sdd specx done 멱등 (spec-14-04)
- **명령**: `bash tests/test-marker-append-guard.sh`
- **결과**: ✅ **5/5 PASS** (A 단위 2건 + B specx done 멱등 + C phase done 멱등 + D 본문 매치 회피)

### 회고 후속 (spec-14-05)

phase 회고 (Opus sub-agent 비판 검증) 에서 발견된 *직접 결과물 결함 6건* 추가 정리 — phase-14 안에서 spec-14-05 로 통합 처리:
- M1: 다중 마커 쌍 + 비대칭 영역 멱등 보강
- M2: 마커 부재 silent → stderr warn + rc=1
- m1: spec_new 정확 토큰 매칭 (백틱 포함)
- M3: 8 파일 헤더 주석 "bash 4.0+" → "bash 3.2+"
- M4: install.sh sed 견고화 (set -e 함정 회피)
- m2: phase-14.md row 양식 정규화

회귀 검증: 10 스위트 (~80건) 모두 PASS.

### 성공 기준 5건 — 모두 충족

| # | 기준 | 결과 |
|:---:|---|:---:|
| 1 | bug-01 재현 시나리오: dead marker 제거 + 안내문 일관 | ✅ |
| 2 | bash 3.2 환경 doctor `bash` 항목 ✅ | ✅ |
| 3 | install.sh 재실행 후 .gitignore 4 라인 각 정확히 1회 | ✅ (22/22) |
| 4 | sdd specx done 두 번 호출 후 done 섹션 1줄 | ✅ (5/5) |
| 5 | 모든 spec 단위 테스트 PASS + 통합 시나리오 4건 PASS | ✅ |

### Icebox (잔재 — phase-14 외)

- `tests/run.sh phase-N` 인프라 부재 (phase 템플릿 boilerplate 정리)
- `docs/harness-kit-bug-01,02.md` archive 처리
- 다른 grep 영역 미한정 점검 (`sdd:859, 941`)
- 회고 메타 메모 5건의 governance 문서 반영
