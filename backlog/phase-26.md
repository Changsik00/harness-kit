...
3. W3: `.claude/settings.json` 의 baseline(mode-managed git push 항목 제외)이 `settings.json.fragment` 와 일치함을 검증하는 sync 테스트 존재 + PASS.
4. 전체 테스트 스위트 PASS(신규 포함), 거버넌스 단어 예산(≤8000) 불변(이 phase 는 governance 문서 거의 미변경).

## 🧩 작업 단위 (phase-FF 전용)

> 본 절은 phase 의 *작업 지도* 입니다. W1/W2/W3 은 각각 1–2 commit 의 작고 가역적인 항목이라 **전부 phase-FF**(spec 산출물 없이 phase base 브랜치에 직접 TDD 커밋, phase-ship PR 에서 일괄 검토 — §11.4 / ADR-004) 로 처리한다. 별도 SPEC 없음.
...
> SPEC 없음 — 본 phase 는 phase-FF 항목만으로 구성. 진행 중 항목이 커지면(§11.3 재검증) SPEC 으로 승격.

### phase-FF 항목

| # | 항목 | 요점 | 연관 모듈 | 예상 commit |
|---|---|---|---|:---:|
| FF1 (W1) | 비가역 가드 경계 박제 | `git -C /x reset --hard`·`rebase -i`·`rm -rf ./dir` 등 narrow 밖 변형이 **의도적 미감지**임을 assert_quiet 로 고정 | `tests/test-stop-rules.sh` | 1 |
| FF2 (W2) | 모드 불명 fail-safe 통일 | state/jq 부재로 mode 불명 시 `_sr_default` 를 block(fail-safe)으로. governed 명시 state 는 warn 회귀 유지 | `sources/hooks/check-irreversible.sh` + 테스트 | 1 |
| FF3 (W3) | settings baseline SSOT sync | `.claude/settings.json` baseline(mode-managed git push 제외) == fragment 검증 sync 테스트 신규 | `sources/claude-fragments/settings.json.fragment`, 신규 `tests/` | 1 |
| FF4 | queue.md phase-24 중복 줄 정리 | `sdd phase done 24` 가 만든 중복 완료 줄 1건 제거(cosmetic) | `backlog/queue.md` | 1 |

> FF2 의 fail-safe 방향(불명→block)은 CLAUDE.md #5 에서 이미 결정됨 → 새 아키텍처 결정 아님(구현+테스트). 그래서 phase-FF 적합.
> FF3 은 다른 파일 영역이나 동일 테마(auto 안전망 잔여)라 본 phase 에 포함.
